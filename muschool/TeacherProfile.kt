...
class TeacherProfile : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        // Add smooth transition (same as student profile)
            ...
            userRef.orderByChild("email").equalTo(currentEmail)
                .addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        if (!snapshot.exists()) {
                            Toast.makeText(this@TeacherProfile, "Profile not found", Toast.LENGTH_SHORT).show()
                            return
                        }

                        for (child in snapshot.children) {
                            Log.d("FIREBASE_TEACHER", child.toString())
                            ...
                            val department = child.child("department").getValue(String::class.java) ?: "N/A"
                            val classes = child.child("class").getValue(String::class.java) ?: "N/A"
                            ...
                            tvDepartment.text = "🏫 Department: $department"
                            tvClasses.text = "📚 Classes: $classes"
                    ...
                    override fun onCancelled(error: DatabaseError) {
                        Toast.makeText(this@TeacherProfile, "Error: ${error.message}", Toast.LENGTH_SHORT).show()
                    }
                })
        } else {
